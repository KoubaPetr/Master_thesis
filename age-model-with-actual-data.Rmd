...
Here, we define an age-structured model to synthesize Frida's data on age-dependent host-parasite characteristics.
...
The following function requires as input the initial age-distribution of susceptible `D` and infected `I` daphnia. The length of the input vectors indicates the number of age classes in days. It also requires the abundance of pathogens `P` in the environment. One also needs to specify the time in days when the simulation should end. 
```{r age-model}
library(foreign)
dataset <- read.spss("/home/petr/Documents/Master_thesis/AgeEffectsVirulence.sav", to.data.frame = TRUE, use.value.labels = FALSE)
infecteds <- dataset$Infected
infection_age <- dataset$InfectionAge
max_lifetime_threshold <- 200     #lifetime of daphnia ~ half a year in captivity

#this should correspond to the state after infecting the fleas, considering one time infection happening to a population heterogenic in age - however this does not make much sense with respect to observing the disease spread. What was the actual experiment like?
#should D contain whole population or just susceptibles? (right now it only has susceptibles)
D_data <- c(rep(0,max_lifetime_threshold))
D_data[1] <- sum((infection_age == 0)*(rep(1,length(infecteds)) - infecteds))
D_data[6] <- sum((infection_age == 5)*(rep(1,length(infecteds)) - infecteds))
D_data[16] <- sum((infection_age == 15)*(rep(1,length(infecteds)) - infecteds))
D_data[31] <- sum((infection_age == 30)*(rep(1,length(infecteds)) - infecteds))

I_data <- c(rep(0,max_lifetime_threshold))
I_data[1] <- sum((infection_age == 0)*infecteds)
I_data[6] <- sum((infection_age == 5)*infecteds)
I_data[16] <- sum((infection_age == 15)*infecteds)
I_data[31] <- sum((infection_age == 30)*infecteds)
...
age.model <-
    ...
    f <- c(rep(0.1, 5),
           ...
           rep(0.02, 15),
           rep(0.02,170))
    ## infectiousness of a burst (ie the number of parasite spores that come out
